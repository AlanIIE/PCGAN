...
class SizeTransformLayer(Layer):
    def __init__(self, image_shape, **kwargs):
        self.image_shape = image_shape
        super(SizeTransformLayer, self).__init__(**kwargs)

    def build(self, input_shape):
        super(SizeTransformLayer, self).build(input_shape)

    def call(self, inputs):
        mask = ktf.image.resize_images(inputs, self.image_shape[1:3], method=ktf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    def get_config(self):
        config = {"number_of_transforms": 0}
        base_config = super(SizeTransformLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))

class MaskLayer(Layer):
    def __init__(self, number_of_transforms, init_image_size, **kwargs):
    ...
    def call(self, inputs):
        ...
        if self.number_of_transforms==11:
            ...
            return inputs[0] * ktf.expand_dims(K.clip(1-K.sum(mask,3),0,1), axis=3)
...
class AffineMaskLayer(Layer):
    ...
    def call(self, inputs):
        ...
        if self.aggregation_fn == 'none':
            ...
            if len(inputs) == 3:
                mask = ktf.transpose(inputs[2], [0, 2, 3, 1])
                mask = ktf.image.resize_images(mask, self.image_size[:2],
    ...
    def get_config(self):
        ...
        base_config = super(AffineMaskLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
def compute_st_distance(kp):
    ...
    return (np.sqrt(st_distance1) + np.sqrt(st_distance2))/2.0


def vertex_from_kp_array(kp_array, border_inc, img_size):
    ...
    vertex = np.array([[min[0],min[1]], [min[0],max[1]], [max[0],min[1]], [max[0], max[1]]])
    return vertex
...
def pose_masks(array2, img_size, use_body_mask, use_mask, path, fat=True):
    kp2 = give_name_to_keypoints(array2)
    masks = []
    st2 = compute_st_distance(kp2)
    empty_mask = np.zeros(img_size)

    body_candidate_names = {'Rhip', 'Rsho', 'Lhip', 'Lsho'}
    body_kp_names = set()
    for cn in body_candidate_names:
        if cn in kp2:
            body_kp_names.add(cn)

    center_of_mass = np.mean(get_array_of_points(kp2, list(body_kp_names)), axis=0, keepdims=True)
    center_of_mass = center_of_mass.astype(int)
    body_mask = mask_from_kp_array(center_of_mass, 0.6 * st2, img_size)
    ...
    def mask_joint(fr, to, shape_idx, inc_to):
        if not check_keypoints_present(kp2, [fr, to]):
            ...
            if not check_keypoints_present(kp2, [fr, to]):
                return empty_mask
        return skimage.measure.grid_points_in_poly(img_size, estimate_polygon(kp2[fr], kp2[to], shape_idx*st2, inc_to, 0.1, 0.2, 0.2)[:, ::-1])

    if fat: # market
        masks.append(mask_joint('Rhip', 'Rkne', 1.4, 0.1)) # legup
        masks.append(mask_joint('Lhip', 'Lkne', 1.4, 0.1)) # legup

        masks.append(mask_joint('Rkne', 'Rank', 1.2, 0.5)) # legdown
        masks.append(mask_joint('Lkne', 'Lank', 1.2, 0.5)) # legdown

        masks.append(mask_joint('Rsho', 'Relb', 0.8, 0.1)) # arm
        masks.append(mask_joint('Lsho', 'Lelb', 0.8, 0.1)) # arm

        masks.append(mask_joint('Relb', 'Rwri', 0.8, 0.5)) # arm
        masks.append(mask_joint('Lelb', 'Lwri', 0.8, 0.5)) # arm
    else: # fasion
        masks.append(mask_joint('Rhip', 'Rkne', 1.0, 0.1)) # legup
        masks.append(mask_joint('Lhip', 'Lkne', 1.0, 0.1)) # legup

        masks.append(mask_joint('Rkne', 'Rank', 1.0, 0.5)) # legdown
        masks.append(mask_joint('Lkne', 'Lank', 1.0, 0.5)) # legdown

        masks.append(mask_joint('Rsho', 'Relb', 0.6, 0.1)) # arm
        masks.append(mask_joint('Lsho', 'Lelb', 0.6, 0.1)) # arm

        masks.append(mask_joint('Relb', 'Rwri', 0.6, 0.5)) # arm
        masks.append(mask_joint('Lelb', 'Lwri', 0.6, 0.5)) # arm

    if use_mask==1:
        gt_mask = np.load(path)
        mask_sum = np.zeros(masks[0].shape)
        for id in range(len(masks)-1):
            masks[id+1] = np.multiply(masks[id+1], gt_mask)
            mask_sum += masks[id+1]
        masks[0] = gt_mask - np.clip(mask_sum,0,1)

    if use_body_mask:
        if use_mask == 1:
            masks.insert(0, gt_mask)
        else:
            masks.insert(0,np.zeros(masks[0].shape))
...
def estimate_vertex(center, st, img_size):

    min = np.min(center, axis=0)
    max = np.min(center, axis=0)
    min -= int(st)
    max += int(st)
    ...
    vetexes = np.array([[min[0],max[1]],[max[0],max[1]],[max[0],min[1]],[min[0],min[1]]])
    return vetexes


def affine_transforms(array1, array2, img_size, use_body_mask):
    ...
    if use_body_mask:
        ...
        body_poly_1 = estimate_vertex(np.mean(body_poly_1, axis=0, keepdims=True), st1, img_size=img_size)
        body_poly_2 = estimate_vertex(np.mean(body_poly_2, axis=0, keepdims=True), st2, img_size=img_size)
    ...
    if len(head_kp_names) != 0:
        # mask vertex
        center_of_mass_kp1 = np.mean(get_array_of_points(kp1, list(head_kp_names)), axis=0, keepdims=True)
        center_of_mass_kp2 = np.mean(get_array_of_points(kp2, list(head_kp_names)), axis=0, keepdims=True)
        center_of_mass_kp1 = center_of_mass_kp1.astype(int)
        center_of_mass_kp2 = center_of_mass_kp2.astype(int)
        head_mask_kp1 = vertex_from_kp_array(center_of_mass_kp1, 0.40 * st2, img_size)
        head_mask_kp2 = vertex_from_kp_array(center_of_mass_kp2, 0.40 * st2, img_size)
        tr = skimage.transform.estimate_transform('affine', src=head_mask_kp2, dst=head_mask_kp1)
        to_transforms(tr.params)
    else:
